...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        sumweighted = self.getWeightedSum([1.0] + inActs)

        return self.sigmoid(sumweighted)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        inActs = [1.0] + inActs
        sumWeighted = self.getWeightedSum(inActs)
        return self.sigmoidDeriv(sumWeighted)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        count = 0
        for elem in inActs:
            upd = alpha * delta * inActs[count]
            self.weights[count] =  self.weights[count] + upd
            tm = tm + abs(upd)
            count+= 1
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        retList = []
        newData = copy.copy(inActs)
        retList.append(inActs)

        for l in list(self.layers):
            newList = []
            for n in l:
                newList.append(n.sigmoidActivation(newData))
            newData = copy.copy(newList)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = example[1][outputNum] - lastLayerOutput[outputNum] #fill
                delta =  gPrime * error #fill
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) #fill
                    nw = 0
                    count = 0
                    for n in range(len(list(nextLayer))):
                        nw = nw + (nextLayer[n].weights[neuronNum + 1] * deltas[0][n])
                    delta = gPrime * nw
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        ret = nnet.feedForward(example[0])[-1]
        same = True
        for valueInd in xrange(len(ret)):
            if round(ret[valueInd]) != example[1][valueInd]:
                same = False
        if not same:
