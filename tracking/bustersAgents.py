...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        ghostProb = 0
        mostLikelyGhostPos = None
        bestAction = None


        for map in livingGhostPositionDistributions:
            for tup in map.items():
                position = tup[0]
                probability = tup[1]
                if probability > ghostProb:
                    mostLikelyGhostPos = position
                    ghostProb = probability

        for act in legal:
            distanceFromSucc = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), mostLikelyGhostPos)
            if bestAction == None or distanceFromSucc < minDistance:
                minDistance = distanceFromSucc
                bestAction = act
        return bestAction
