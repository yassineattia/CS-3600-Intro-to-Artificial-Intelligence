...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        beliefUp = util.Counter()
        if noisyDistance is not None:
            for position in list(self.legalPositions):
                manhattanDistance = util.manhattanDistance(position, pacmanPosition)
                if emissionModel[manhattanDistance] > 0:
                    proba = emissionModel[manhattanDistance] * self.beliefs[position]
                    beliefUp[position] = proba
        else:
            beliefUp[self.getJailPosition()] = 1
            self.beliefs = beliefUp
            return
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in list(self.legalPositions):
            distributionNew = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            for pair in distributionNew.items():
                position = pair[0]
                probability = pair[1]
                ret[position] = ret[position] + probability * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        listOfparticles = []
        for numC in range(self.numParticles):
            bound = numC % len(self.legalPositions)
            listOfparticles.append(self.legalPositions[bound])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            ...
            for particule in self.particles:
                manhDis = util.manhattanDistance(particule, pacmanPosition)
                beliefUp[particule] = beliefUp[particule] + emissionModel[manhDis]
            self.beliefs = beliefUp
        else:
            #noisyDistance is None, ghost in jail particles are updated
            self.beliefs = util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"2
        newBelief = []
        #print(newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)))
        for oldPosititon in self.particles:
            newBelief.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPosititon))))
        self.particles = newBelief
